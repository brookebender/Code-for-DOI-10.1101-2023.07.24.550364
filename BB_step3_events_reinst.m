...
EventFile = 'MP8_6_1_Reinst-events.mat'; 
TracesFile = 'MP8_6_1_Reinst-traces.mat';
...
sessionstart=TTLGP1(1,1);
timeofcues=TTLGP4(:,1)-sessionstart;    %Creates array of cue start times after behavioral session starts
cues15=find(timeofcues(:,1)<900);
cues30=find(timeofcues(:,1)>900 & timeofcues(:,1)<1800);
cues45=find(timeofcues(:,1)>1800 & timeofcues(:,1)<2700);
cues60=find(timeofcues(:,1)>2700 & timeofcues(:,1)<3600);    %Separates cues into 15-min bins
TS.cues15=TTLGP4(cues15,1);
TS.cues30=TTLGP4(cues30,1);
TS.cues45=TTLGP4(cues45,1);
TS.cues60=TTLGP4(cues60,1);
else
end
...
timeofisolongcues=TS.IsolatedLongCues(:,1)-sessionstart;
isolongcues15=find(timeofisolongcues(:,1)<900);
isolongcues30=find(timeofisolongcues(:,1)>900 & timeofisolongcues(:,1)<1800);
isolongcues45=find(timeofisolongcues(:,1)>1800 & timeofisolongcues(:,1)<2700);
isolongcues60=find(timeofisolongcues(:,1)>2700 & timeofisolongcues(:,1)<3600);    %Separates isolongcues into 15-min bins
TS.isolongcues15=TS.IsolatedLongCues(isolongcues15,1);
TS.isolongcues30=TS.IsolatedLongCues(isolongcues30,1);
TS.isolongcues45=TS.IsolatedLongCues(isolongcues45,1);
TS.isolongcues60=TS.IsolatedLongCues(isolongcues60,1);  %Makes arrays for isolated long cues in 15-minute bins
else
end
else
end
