    ...
    for q=1:length(twocolor)
        ...
        for e = 1:size(fnEventType,1)                                               %cycle through event type (e)
            ...
            for r = 1:length(fnRegion);                              %cycle through regions (r)
                    ...
                    tempEventT = transpose(tempEvent);        %put indexed trace data into a temporary vector
                    tempbaseline=groupData.(fnID{a}).eventMean.(twocolor{q}).(fnEventType{e}).pre.(fnRegion{r});
                    ind = ~isnan(tempbaseline);
                    tempbaseline=tempbaseline(ind);             %removes any NaNs at the end
                    temppremean=mean(tempbaseline);             %Calculates the average of the "pre" window
                    tempData=tempEventT - temppremean;           %Subtracts out the "pre" window from tempData
